...
export function registerExportIPC(_mainWindow?: BrowserWindow | null): void {
  ...
  ipcMain.handle(
    ...
    async (
      _e,
      params: { taskId: string; targetDir: string; stripObsidianSyntax?: boolean },
  ...
  ipcMain.handle(
    'export:toLogseq',
    async (_e, params: { taskId: string }): Promise<ExportResult> => {
      try {
  ...
  ipcMain.handle(
    'export:notion:testConnection',
    async (_e, params: NotionTestConnectionParams): Promise<NotionTestConnectionResult> => {
      try {
