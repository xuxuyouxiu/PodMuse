...
export async function processPodcast(
  ...
  if (isLocalFile) {
    ...
    if (!platformInfo) {
      step({ step: 1, title: '解析页面', subtitle: '不支持的平台', status: 'error', detail: '暂不支持该平台，请使用本地文件方式' })
      log('  ❌ 不支持的平台链接')
      return null
    }
    ...
    try {
      ...
      if (result.type === 'pre_transcribed' && result.transcript) {
        ...
        step({ step: 1, title: '解析页面', subtitle: title || '未知标题', status: 'done', detail: `${platformInfo.name}：已获取字幕/转写` })
        log(`  标题: ${title || '未知'}（使用平台字幕，${preTranscript.length} 字）`)

        check()
        step({ step: 2, title: '下载音频', subtitle: '跳过（使用字幕）', status: 'done', progress: 100, detail: '已获取平台字幕，无需下载音频' })
        log('  [2/5] 使用平台字幕，跳过下载')
        audioPath = ''
        ...
        if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 1024) {
          ...
          step({ step: 1, title: '解析页面', subtitle: title || '未知标题', status: 'done', detail: `${platformInfo.name}：音频已缓存` })
        } else {
          step({ step: 1, title: '解析页面', subtitle: '提取音频', status: 'running', detail: `使用 yt-dlp 提取 ${platformInfo.name} 音频...` })
          try {
            const extracted = await extractAudioWithYtDlp(
              ytDlp.path!, result.audioUrl!, tmp, audioName,
              (msg) => log(`  [yt-dlp] ${msg}`), signal,
            )
            ...
            if (signal?.aborted) throw e
            step({ step: 1, title: '解析页面', subtitle: '提取失败', status: 'error', detail: errMsg(e) })
            log(`  ❌ yt-dlp 提取失败: ${errMsg(e)}`)
            return null
          }
          step({ step: 1, title: '解析页面', subtitle: title || '未知标题', status: 'done', detail: `${platformInfo.name}：音频提取完成` })
        }
        log(`  标题: ${title || '未知'}`)
          ...
          try {
            ...
            if (subPath) {
              ...
              if (subText && subText.length > 50) {
                ...
                step({ step: 2, title: '下载音频', subtitle: '跳过（使用字幕）', status: 'done', progress: 100, detail: `已获取 YouTube 字幕 ${preTranscript.length} 字` })
                // 清理字幕临时文件
        ...
        if (!preTranscript) {
          ...
          const stat = fs.statSync(audioPath)
          step({ step: 2, title: '下载音频', subtitle: `${(stat.size / 1048576).toFixed(1)} MB`, status: 'done', progress: 100, detail: `yt-dlp 提取完成` })
  ...
  if (preTranscript) {
    ...
    step({ step: 3, title: '语音转文字', subtitle: `平台字幕 ${transcript.length} 字`, status: 'done', progress: 100, detail: '使用平台字幕，跳过 Whisper' })
