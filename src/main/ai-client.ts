...
**G. 术语词典**（必选）
作为术语名称索引，每个术语用 [[术语名]] 格式包裹。术语的完整解释放在对应的 CARD-TERM 卡片中。列出的每一个术语都必须生成对应的 CARD-TERM 卡片，不允许遗漏。在正文其他章节首次提及这些术语时，也用 [[术语名]] 格式包裹。
格式参考：
\`\`\`
# 术语词典（索引）
...
4. 所有 wiki-link 使用 [[xxx]] 格式包裹。在正文中首次提及术语词典收录的术语时，也用 [[术语名]] 格式包裹。
