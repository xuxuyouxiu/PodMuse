...
**A. 主要内容概览**（必选）
用结构化的列表概括本期播客涉及的所有主要内容点。确保覆盖从头到尾的所有重要内容，不遗漏后半部分。
格式参考：
\`\`\`
# 本期主要内容
- 要点1：简述
...
**B. 核心观点提炼**
当播客有明确的观点表达时使用（如访谈、评论、演讲类）。按人物分别列出各自的核心观点。如果是新闻资讯类，改为按事件提炼要点。
格式参考：
\`\`\`
# 核心观点
## [人物/事件名]
...
**C. 事件详情与深度分析**
当需要对每条新闻/事件展开详细分析时使用。特别适合新闻资讯类型——每条新闻独立展开，包含事件摘要（50-100字）、影响分析、关键数据、相关方及立场。
格式参考：
\`\`\`
# 事件详情
## 事件标题
...
**D. 关键对话还原**
当播客中有精彩的对话交锋、问答互动时使用。用引用格式（>）保留原话，标注对话主题。**新闻资讯、每日简报等不涉及对话场景的内容类型不要使用此模块。**
格式参考：
\`\`\`
# 关键对话还原
> 主题：xxx
...
⚠️ 重要：在正文其他章节（如事件详情、核心观点等）中首次提及这些术语时，也必须使用 [[术语名]] 格式包裹，这样读者在阅读正文时就能直接跳转到术语解释，无需手动翻到底部查找。
格式参考：
\`\`\`
# 术语词典（索引）
...
**H. 关联实体索引**
在笔记末尾汇总本期涉及的所有人物、项目、概念的 wiki-link。
⚠️ 重要规则：
1. **索引与卡片严格对应**：索引中列出的每一个条目都必须有对应的 CARD 卡片（CARD-PEOPLE / CARD-PROJECT / CARD-CONCEPT），没有卡片的条目不要列入索引。
2. **分类互斥**：人名只能放在「关联人物」，公司/品牌/产品只能放在「关联项目」，抽象概念/理论/现象只能放在「关联概念」。绝对不要把人名放到关联概念里。
3. **同一实体只出现一种卡片**：如果一个实体已作为 CARD-CONCEPT 输出，就不要再生成 CARD-TERM，反之亦然。优先选择最匹配的类型。
格式参考：
\`\`\`
# 关联人物
- [[人物名]]
...
核心原则：
1. 输出为纯 Markdown 格式，按照下方的「输出格式说明」组织内容，不要添加格式说明以外的任何解释。
2. **重要：必须覆盖播客从头到尾的所有重要内容**，不要只提炼前半部分。
3. 语言使用简体中文（播客中的英文内容保留原文）。
4. 所有 wiki-link 使用 [[xxx]] 格式包裹。
5. **人物卡片硬性筛选（最高优先级）：** 绝对不要为没有公开知名度的人物生成 CARD-PEOPLE 卡片。如果不确定某人是否符合条件，跳过他。
6. **严禁内容重复：** 同一事件/新闻/观点在全文只出现一次。如果两条新闻内容高度相似，合并为一条。绝对不要生成两个相同标题或相同内容的事件章节。
7. **正文内术语链接：** 在正文（事件详情、核心观点等章节）中首次提及术语词典收录的术语时，必须用 [[术语名]] 格式包裹，方便读者直接跳转查看解释。
8. **实体索引与卡片严格对应：** 关联人物/项目/概念索引中列出的每一个条目都必须有对应的 CARD 卡片。不要列出没有卡片的条目。同时，同一实体只能出现在一种卡片类型中（不要在概念和项目里重复出现同一个实体）。`
...
- ⚠️ **严禁重复**：同一事件只生成一个章节，内容相似的事件合并为一个，不要分成两个章节重复输出`
      break
    case 'article':
    case 'tutorial':
      typeGuidance = `
