...
# 关键对话还原
（⚠️ 如果上方要求指明"新闻资讯类型"，则完全跳过此章节，不要输出本节标题和内容）
...
8. **新闻资讯类型——强制规则**：
   - category 必须填「每日资讯」，不要选择其他类别。
   - **完全跳过「关键对话还原」章节**，不要输出该标题和任何内容。新闻资讯不涉及对话场景。
   - **信息密度要高**：不要用一两句话概括整条新闻，要提取充实的信息。
9. **每条新闻/事件必须包含**：
   - 事件摘要：50-100 字描述事件核心内容，保留关键细节（谁、做了什么、在哪里、什么时候）
   - 影响分析：对社会、行业、个人的潜在影响和后续发展预判
   - 关键数据：新闻中提到的所有具体数字、金额、百分比、日期，必须完整保留
   - 相关方：涉及的机构、政府部门、企业、关键人物及其立场
   - 核心观点：提炼 2-3 条该事件的核心要点
10. **保留新闻细节**：
    - 重要的新闻引述和表态用引用格式（>）保留原文
    - 事件背景信息：帮助理解新闻的前因后果
    - 如果一期包含多条新闻，每条都要独立、详细地展开，不要合并概括
11. **关联分析**：如果多条新闻之间存在关联（同一事件的不同方面、因果关系），在「关联延伸」中说明`
      break
    case 'article':
    case 'tutorial':
      specificRequirements = `
